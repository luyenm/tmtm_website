...
- [Proxying API Requests in Development](#proxying-api-requests-in-development)
  - ["Invalid Host MainComponent" Errors After Configuring Proxy](#invalid-host-header-errors-after-configuring-proxy)
...
Now you can rename `src/App.css` to `src/App.scss` and update `src/MainComponent.jsnent.js` to import `src/App.scss`.
...
Import required reactstrap components within `src/MainComponent.jsnent.js` file or your custom component files:

```js
import { Button } from 'reactstrap';
```

Now you are ready to use the imported reactstrap components within your component hierarchy defined in the render method. Here is an example [`MainComponentmponent.js`](https://gist.githubusercontent.com/zx6658/d9f128cd57ca69e583ea2b5fea074238/raw/a56701c142d0c622eb6c20a457fbc01d708cb485/MainComponentmponent.js) redone using reactstrap.
...
4. Add `// @flow` to any files you want to type check (for example, to `src/MainComponent.jsnent.js`).
...
To try it, delete all the code in `src/MainComponent.jsnent.js` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
### "Invalid Host MainComponent" Errors After Configuring Proxy
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `App.test.js` and `MainComponentmponent.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
